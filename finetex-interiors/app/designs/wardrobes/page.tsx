...
import { wardrobeDesigns as wardrobeImages } from "@/lib/designs";

const wardrobeDesigns = wardrobeImages.map((image, index) => ({
  image,
  alt: `FINETEX INTERIORS wardrobe design ${index + 1}`,
}));
...
export default function WardrobesPage() {
  ...
  const showNext = useCallback(() => {
    setSelectedImage((current) => {
      ...
      return current === wardrobeDesigns.length - 1
        ? 0
        : current + 1;
    });
  }, []);
    ...
    <>
          ...
          <div className="container py-20 sm:py-28">
            ...
            <div className="mt-12 max-w-3xl">
              ...
              <p className="mt-6 max-w-2xl text-base leading-8 text-white/65 sm:text-lg">
                Explore wardrobe styles and ideas from FINETEX INTERIORS.
              </p>
            </div>
          </div>
        </section>
            ...
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {wardrobeDesigns.map((design, index) => (
                <button
                  ...
                  aria-label={`View wardrobe design ${index + 1}`}
                >
                  <div
                    className={`relative ${
                      index === 0
      ...
      {selectedImage !== null && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Wardrobe design preview"
        ...
        >
          ...
          <button
            ...
            aria-label="Previous wardrobe design"
          >
            <ChevronLeft size={24} />
          </button>
          ...
          >
            <Image
              src={wardrobeDesigns[selectedImage].image}
              alt={wardrobeDesigns[selectedImage].alt}
              fill
              sizes="100vw"
          ...
          <button
            ...
            aria-label="Next wardrobe design"
          >
          ...
          <div className="absolute bottom-5 left-1/2 -translate-x-1/2 rounded-full bg-white/10 px-4 py-2 text-xs font-medium text-white/70 backdrop-blur">
            {selectedImage + 1} / {wardrobeDesigns.length}
          </div>
        </div>
      )}
